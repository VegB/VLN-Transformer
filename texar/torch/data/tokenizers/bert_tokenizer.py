...
class BERTTokenizer(PretrainedBERTMixin, TokenizerBase):
    ...
    def __init__(self,
        ...
        if self.pretrained_model_dir is not None:
            if hparams is not None and 'vocab_file' in self.hparams.keys():
                vocab_file = hparams['vocab_file']
            else:
                vocab_file = os.path.join(self.pretrained_model_dir, self._VOCAB_FILE_NAMES['vocab_file'])
    ...
    def encode_text(self,
        ...
        for id in input_ids[1:]:
            ...
            if id == eos_id:
                cur_id += 1
        sentence_ids = sentence_ids + [-1] * (max_seq_length - len(sentence_ids))
        sentence_num = cur_id
